...
namespace DataLayer
{
    public sealed class DesktopDataContext : DbContext
    {
        ...
        public DbSet<BaseValveWithShearPin> BaseValveWithShearPin { get; set; }
...
#else
            optionsBuilder.UseSqlite($@"Filename = {Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)}\SupervisionDataVol2.sqlite", options => options.MigrationsAssembly(Assembly.GetExecutingAssembly().FullName));
