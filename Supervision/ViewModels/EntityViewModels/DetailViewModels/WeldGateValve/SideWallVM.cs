...
{
    public class SideWallVM : ViewModelBase
    {
        private readonly DataContext db;
        private IList<SideWall> allInstances;
        ...
        public string Material
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is SideWall item && item.MetalMaterial.Material != null)
                    {
        ...
        public string Melt
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is SideWall item && item.MetalMaterial.Melt != null)
                    {
        ...
        public string Certificate
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is SideWall item && item.MetalMaterial.Certificate != null)
                    {
        ...
        public IList<SideWall> AllInstances
        {
            get => allInstances;
            set
            {
        ...
        public static SideWallVM LoadVM(DataContext context)
        {
            SideWallVM vm = new SideWallVM(context);
        ...
        private async Task UpdateList()
        {
            ...
            {
                IsBusy = true;
                AllInstances = new ObservableCollection<SideWall>();
        ...
        private async Task AddNewItem()
        {
            ...
            {
                IsBusy = true;
                SelectedItem = await repo.AddAsync(new SideWall());
                var tcpPoints = await repo.GetTCPsAsync();
                var records = new List<SideWallJournal>();
                foreach (var tcp in tcpPoints)
                {
                    var journal = new SideWallJournal(SelectedItem, tcp);
                    if (journal != null)
                        records.Add(journal);
                }
        ...
        private async Task CopySelectedItem()
        {
            if (SelectedItem != null)
            {
                try
                {
                    ...
                    var copy = await repo.AddAsync(new SideWall(temp));
                    var jour = new ObservableCollection<SideWallJournal>();
                    foreach (var i in temp.SideWallJournals)
                    {
                        var record = new SideWallJournal(copy.Id, i);
                        jour.Add(record);
                    }
        ...
        private void EditSelectedItem()
        {
            if (SelectedItem != null)
            {
                ...
                {
                    DataContext = SideWallEditVM.LoadVM(SelectedItem.Id, SelectedItem, db)
                };
            }
        ...
        public SideWallVM(DataContext context)
        {
            db = context;
