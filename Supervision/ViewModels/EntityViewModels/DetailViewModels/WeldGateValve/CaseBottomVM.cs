    ...
    public class CaseBottomVM : ViewModelBase
    {
        ...
        public string DN
        {
            ...
            set
            {
                ...
                {
                    if (obj is CaseBottom item && item.DN != null)
                    {
        ...
        public string Material
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CaseBottom item && item.Material != null)
                    {
        ...
        public string Melt
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CaseBottom item && item.Melt != null)
                    {
        ...
        private async Task AddNewItem()
        {
            try
            {
                ...
                //CaseBottom caseBottomEmpty = new CaseBottom();
                //caseBottomEmpty.MetalMaterial = Materials[0];
                //caseBottomEmpty.MetalMaterialId = 1;
        ...
        private async Task CopySelectedItem()
        {
            if (SelectedItem != null)
            {
                try
                {
                    ...
                    var copy = await repo.AddAsync(new CaseBottom(temp, number));
                    var jour = new ObservableCollection<CaseBottomJournal>();
                    if (temp.CaseBottomJournals != null)
                    {
            ...
            if (SelectedItem != null)
            {
                try
                {
                    ...
                    if (System.Int32.TryParse(Input, out int x))
                    {
                        ...
                        {
                            var temp = await repo.GetByIdIncludeAsync(SelectedItem.Id);
                            CaseBottom detail = new CaseBottom(temp);
                            if (System.Int32.TryParse(detail.Number, out int numb))
                            {
                                for (int y = 0; y < x; y++)
                                {
                                    ...
                                    var copy = await repo.AddAsync(new CaseBottom(detail));
                                    var jour = new ObservableCollection<CaseBottomJournal>();
                                    if (temp.CaseBottomJournals != null)
                                    {
                                        foreach (var i in temp.CaseBottomJournals)
            ...
            if (SelectedItem != null)
            {
                ...
                if (MainViewModel.HelpMode == true)
                {
                    MessageBox.Show("Поля внутри данного раздела заполняются в соответствии с их описанием, без добавления \"серт.\", " +
                        ...
                        "После заполнения нужных полей и закрытия операций, необходимо нажать кнопку \"Сохранить\".", "Раздел \"Редактирование днища\"");
                }
            }
