...
{
    public class CoverFlangeVM : ViewModelBase
    {
        private readonly DataContext db;
        private IList<CoverFlange> allInstances;
        ...
        public string DN
        {
            ...
            set
            {
                ...
                {
                    if (obj is CoverFlange item && item.DN != null)
                    {
        ...
        public string Number
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.Number != null)
                    {
        ...
        public string Drawing
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.Drawing != null)
                    {
        ...
        public string Status
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.Status != null)
                    {
        ...
        public string Material
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.Material != null)
                    {
        ...
        public string Melt
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.Melt != null)
                    {
        ...
        public string Certificate
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.Certificate != null)
                    {
        ...
        public IList<CoverFlange> AllInstances
        {
            get => allInstances;
            set
            {
        ...
        public static CoverFlangeVM LoadVM(DataContext context)
        {
            CoverFlangeVM vm = new CoverFlangeVM(context);
        ...
        private async Task UpdateList()
        {
            ...
            {
                IsBusy = true;
                AllInstances = new ObservableCollection<CoverFlange>();
        ...
        private async Task AddNewItem()
        {
            ...
            {
                IsBusy = true;
                SelectedItem = await repo.AddAsync(new CoverFlange());
                var tcpPoints = await repo.GetTCPsAsync();
                var records = new List<CoverFlangeJournal>();
                foreach (var tcp in tcpPoints)
                {
                    var journal = new CoverFlangeJournal(SelectedItem, tcp);
                    if (journal != null)
                        records.Add(journal);
                }
        ...
        private async Task CopySelectedItem()
        {
            if (SelectedItem != null)
            {
                try
                {
                    ...
                    string number = Microsoft.VisualBasic.Interaction.InputBox("Введите номер:");
                    var copy = await repo.AddAsync(new CoverFlange(temp, number));
                    var jour = new ObservableCollection<CoverFlangeJournal>();
                    foreach (var i in temp.CoverFlangeJournals)
                    {
                        var record = new CoverFlangeJournal(copy.Id, i);
                        jour.Add(record);
                    }
        ...
        private async Task GroupCopySelectedItem()
        {
            if (SelectedItem != null)
            {
                ...
                {
                    IsBusy = true;
                    string Input = Microsoft.VisualBasic.Interaction.InputBox("Введите количество деталей, нумерация продолжится автоматически относительно выбранной детали:");
                    if (System.Int32.TryParse(Input, out int x))
                    {
                        ...
                        {
                            var temp = await repo.GetByIdIncludeAsync(SelectedItem.Id);
                            CoverFlange detail = new CoverFlange(temp);
                            if (System.Int32.TryParse(detail.Number, out int numb))
                            {
                                for (int y = 0; y < x; y++)
                                {
                                    ...
                                    var copy = await repo.AddAsync(new CoverFlange(detail));
                                    var jour = new ObservableCollection<CoverFlangeJournal>();
                                    if (temp.CoverFlangeJournals != null)
                                    {
                                        foreach (var i in temp.CoverFlangeJournals)
                                        {
                                            var record = new CoverFlangeJournal(copy.Id, i);
        ...
        private void EditSelectedItem()
        {
            if (SelectedItem != null)
            {
                ...
                {
                    DataContext = CoverFlangeEditVM.LoadVM(SelectedItem.Id, SelectedItem, db)
                };
                if (MainViewModel.HelpMode == true)
                {
                    MessageBox.Show("Поля внутри данного раздела заполняются в соответствии с их описанием, без добавления \"серт.\", " +
                        ...
                        "После заполнения нужных полей и закрытия операций, необходимо нажать кнопку \"Сохранить\".", "Раздел \"Редактирование фланца\"");
                }
            }
        ...
        public CoverFlangeVM(DataContext context)
        {
            db = context;
