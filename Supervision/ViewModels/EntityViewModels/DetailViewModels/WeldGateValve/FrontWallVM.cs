...
{
    public class FrontWallVM : ViewModelBase
    {
        private readonly DataContext db;
        private IList<FrontWall> allInstances;
        ...
        public IList<FrontWall> AllInstances
        {
            get => allInstances;
            set
            {
        ...
        public static FrontWallVM LoadVM(DataContext context)
        {
            FrontWallVM vm = new FrontWallVM(context);
        ...
        private async Task UpdateList()
        {
            ...
            {
                IsBusy = true;
                AllInstances = new ObservableCollection<FrontWall>();
        ...
        private async Task AddNewItem()
        {
            ...
            {
                IsBusy = true;
                SelectedItem = await repo.AddAsync(new FrontWall());
                var tcpPoints = await repo.GetTCPsAsync();
                var records = new List<FrontWallJournal>();
                foreach (var tcp in tcpPoints)
                {
                    var journal = new FrontWallJournal(SelectedItem, tcp);
                    if (journal != null)
                        records.Add(journal);
                }
        ...
        private async Task CopySelectedItem()
        {
            if (SelectedItem != null)
            {
                try
                {
                    ...
                    var copy = await repo.AddAsync(new FrontWall(temp));
                    var jour = new ObservableCollection<FrontWallJournal>();
                    foreach (var i in temp.FrontWallJournals)
                    {
                        var record = new FrontWallJournal(copy.Id, i);
                        jour.Add(record);
                    }
        ...
        private void EditSelectedItem()
        {
            if (SelectedItem != null)
            {
                ...
                {
                    DataContext = FrontWallEditVM.LoadVM(SelectedItem.Id, SelectedItem, db)
                };
            }
        ...
        public FrontWallVM(DataContext context)
        {
            db = context;
