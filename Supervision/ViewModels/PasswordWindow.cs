...
namespace Supervision.Views
{
    public partial class PasswordWindow : Form
    {
        ...
        public PasswordWindow(MainViewModel mainView)
        {
            InitializeComponent();
            InitializeMyControl();
        ...
        {
            if (textBox1.Text.Equals("пткпенза58"))
            {
                mainView.TCPKey = true;
                Page page = new TCPPage();
                page.DataContext = mainView;
                mainView.SlowOpacity(page);
                this.Hide();
            }
            else if (textBox1.Text.Equals("сервисныекоманды"))
            {
                mainView.ServiceKey = true;
                var w = new ServiceWindow();
                w.DataContext = ServiceCommandsVM.LoadVM();
                this.Hide();
            }
            else if (textBox1.Text.Equals("пензаперсонал20"))
            {
